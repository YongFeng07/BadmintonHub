...
/// failed-login lockout and account-status blocking.
/// </summary>
public class AuthServiceTests
{
    ...
    public async Task Authenticate_UnknownEmail_ReturnsGenericErrorAndLogsAttempt()
    {
        ...
        var (user, error) = await service.AuthenticateAsync("nobody@test.local", "Whatever1", null);
    ...
    public async Task Authenticate_WrongPassword_IncrementsFailedCounter()
    {
        ...
        var (user, error) = await service.AuthenticateAsync("member@test.local", "WrongPass1", null);
    ...
    [Fact]
    public async Task Authenticate_FiveFailures_LocksAccountEvenWithCorrectPassword()
    {
        using var db = TestDb.Create();
        var service = CreateService(db);
        ...
        var (user, error) = await service.AuthenticateAsync("member@test.local", "Member@123", null);
        Assert.Null(user);
