...
public class AccountController : Controller
{
    ...
    public async Task<IActionResult> Register(RegisterViewModel model)
    {
        ...
        var sent = await _emailService.SendAsync(user.Email, "Verify your SportHub account",
    ...
    public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
    {
        ...
        if (resetLink != null)
        {
            var user = await _db.Users.FirstOrDefaultAsync(u => u.Email.ToLower() == model.Email.Trim().ToLowerInvariant());
            if (user != null)
                sent = await _emailService.SendAsync(user.Email, "Reset your SportHub password",
