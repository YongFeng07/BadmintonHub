...
{
    /// <summary>
    /// Returns the status of every hourly slot of every court (or one court) for a date,
    /// combining availability records, reservations and court/facility status.
    /// </summary>
    Task<List<SlotStatusViewModel>> GetAvailabilityForDateAsync(DateOnly date, int? courtId = null);
